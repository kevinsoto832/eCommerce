...
if ( ! defined( 'ECOMMERCE_VERSION' ) ) {
	...
	define( 'ECOMMERCE_VERSION', '0.1.0' );
}
...
if ( ! function_exists( 'ecommerce_setup' ) ) :
	...
	function ecommerce_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => __( 'Primary', 'ecommerce' ),
				'menu-2' => __( 'Footer Menu', 'ecommerce' ),
			)
		);
...
function ecommerce_widgets_init() {
	register_sidebar(
		array(
			'name'          => __( 'Footer', 'ecommerce' ),
			'id'            => 'sidebar-1',
			'description'   => __( 'Add widgets here to appear in your footer.', 'ecommerce' ),
...
function ecommerce_scripts() {
	...
	wp_enqueue_script( 'ecommerce-script', get_template_directory_uri() . '/js/script.min.js', array(), ECOMMERCE_VERSION, true );
 ...
 * @return array
 */
function ecommerce_tinymce_add_class( $settings ) {
	$settings['body_class'] = 'block-editor-block-list__layout';
	return $settings;
}
add_filter( 'tiny_mce_before_init', 'ecommerce_tinymce_add_class' );
